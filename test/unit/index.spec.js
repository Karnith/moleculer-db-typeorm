...
describe("Test entityChanged method", () => {
	...
	it("should call `beforeEntityCreate` event", () => {
		return service
			.beforeEntityChange("create", {}, ctx)
			.catch(protectReject)
			.then(() => {
				expect(service.schema.beforeEntityCreate).toHaveBeenCalledTimes(
					1
				);
				expect(service.schema.beforeEntityCreate).toHaveBeenCalledWith(
					{},
					ctx
				);
			});
	...
	it("should call `beforeEntityUpdate` event", () => {
		return service
			.beforeEntityChange("update", {}, ctx)
			.catch(protectReject)
			.then(() => {
				expect(service.schema.beforeEntityUpdate).toHaveBeenCalledTimes(
					1
				);
				expect(service.schema.beforeEntityUpdate).toHaveBeenCalledWith(
					{},
					ctx
				);
			});
	...
	it("should call `beforeEntityRemove` event", () => {
		return service
			.beforeEntityChange("remove", {}, ctx)
			.catch(protectReject)
			.then(() => {
				expect(service.schema.beforeEntityRemove).toHaveBeenCalledTimes(
					1
				);
				expect(service.schema.beforeEntityRemove).toHaveBeenCalledWith(
					{},
					ctx
				);
			});
...
describe("Test sanitizeParams method", () => {
	...
	it("should convert excludeFields to array", () => {
		const res = service.sanitizeParams(ctx, {
			excludeFields: "name votes author",
		});
	...
	describe("Test excludeFields", () => {
		describe("Test with object", function () {
			...
			it("should return expected - fields & excludeFields", () => {
				const ctx = {
					params: { fields: ["a"], excludeFields: ["a.c"] },
				};
				return service
					.transformDocuments(ctx, ctx.params, docs)
					.then((res) => {
			...
			it("should return expected - fields & excludeFields - deep", () => {
				const ctx = {
					params: { fields: ["a"], excludeFields: ["a.d.e.f"] },
				};
				return service
					.transformDocuments(ctx, ctx.params, docs)
					.then((res) => {
