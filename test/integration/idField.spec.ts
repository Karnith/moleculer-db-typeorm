...
describe("Test CRUD methods with idField", () => {
	...
	const posts = [
		...
		},
		{
			myID: "0000002",
			title: "My post",
			content: "This is the end! Good bye!",
			votes: 0,
		},
		{
			myID: "0000003",
			title: "Final post",
			content: "A final good bye!",
			votes: 123,
		},
	];
	...
	it("should throw an error", () => {
		...
		return broker.call("posts.create", posts[0]).catch((err) => {
			expect(err).toEqual(
				new Error(
					'Can\'t insert key "0000000", it violates the unique constraint'
				)
			);
		});
	});
	...
	it("should throw an error", () => {
		...
		return broker
			...
			.catch((err) => {
				expect(err).toEqual(
					new Error(
						'Can\'t insert key "0000001", it violates the unique constraint'
					)
				);
			});
	});
	...
	it("should return with the entity by ID", () => {
		return broker
			.call("posts.get", { id: posts[1].myID })
	...
	test("should throw an error", () => {
		...
		return broker
			...
			.catch((err) => {
				expect(err).toEqual(
					new Error(
						'Can\'t insert key "0000003", it violates the unique constraint'
					)
				);
			});
	});
	...
	it("should remove entity by ID", () => {
		return broker
			.call("posts.remove", { id: posts[2].myID })
			.then((res) => {
				expect(res).toBe(1);
			});
	});
